...
class RemoveFavoriteUseCaseTest {
    ...
    @Test
    fun invoke_delegatesToRepositoryRemoveFavorite() = runTest {
        val useCase = RemoveFavoriteUseCase(fakeRepository)
        val item = FavoriteItem(itemId = "item_20", type = FavoriteType.ALBUM)

        useCase(item)

        assertEquals(1, fakeRepository.removedItems.size)
        assertEquals(item, fakeRepository.removedItems.first())
    }
}
